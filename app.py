...
st.set_page_config(page_title="学习DDL管理 Pro", layout="wide")
    ...
    if total > 0:
        ...
        try:
            ...
            if not df_temp.empty:
                ...
                urgent = df_temp[(df_temp["截止日期"] >= datetime.now()) & (df_temp["截止日期"] <= datetime.now() + timedelta(days=3))]
                if not urgent.empty:
            ...
            if not api_key:
                ...
                with st.spinner("AI解析中..."):
                    try:
                        ...
                        prompt = f"提取学习任务信息。规则：1.提取课程名称 2.提取截止日期(转为YYYY-MM-DD) 3.提取描述。只返回JSON: {{'course': '', 'deadline': '', 'notes': ''}} 文本：{user_input}"
...
with tab2:
    ...
    if search_keyword:
        df_display = df_display[df_display["课程/科目"].str.contains(search_keyword, na=False) | df_display["描述"].str.contains(search_keyword, na=False)]
    if filter_tag != "全部":
    ...
    if not df_display.empty:
        ...
        if not df_display.empty:
            ...
            st.caption("💡 点击下方【状态】列切换 未完成/已完成")
        ...
        if df_chart.empty:
            ...
            try:
                ...
                if not df_chart.empty:
                    ...
                    fig = px.bar(count_df, x="星期", y="任务数量", title="未来30天 DDL 分布", color="任务数量", color_continuous_scale="Reds", text="任务数量")
...
with tab5:
    ...
    uploaded = st.file_uploader("支持 PDF / TXT", type=["pdf", "txt"])
    if uploaded:
        text = ""
...
with tab6:
    ...
    st.caption("🔴红色(≥3项)  🟠橙色(1-2项)  🟢绿色(无任务)")
...
st.divider()
st.caption("💡 功能：AI解析 | 红绿灯预警 | 月视图 | 数据备份 | 批量管理")    # 尝试常见格式
    try:
        dt = datetime.strptime(date_str, "%Y-%m-%d")
        return dt.strftime("%Y-%m-%d")
    except:
