...
with tab6:
    ...
    if not df_cal.empty:
        try:
            df_cal["截止日期_dt"] = pd.to_datetime(df_cal["截止日期"], errors='coerce')
            df_cal = df_cal.dropna(subset=["截止日期_dt"])
            if not df_cal.empty:
...
st.divider()
st.caption("💡 Pro 终极版：AI解析 | 红绿灯预警 | 月视图 | 图片OCR | 数据备份 | 暗黑主题")    st.session_state.df.to_csv(DATA_FILE, index=False, encoding="utf-8-sig")
